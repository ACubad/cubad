...
export function TierScopeFields({ tracks, subjects }: { tracks: { id: string; title: Bi }[]; subjects: { id: string; title: Bi }[] }) {
  ...
  return (
    <>
      ...
      {scopeType !== "all" && <label className="flex flex-col gap-1 text-sm">{scopeType === "track" ? "Target track" : "Target subject"}<select name="scope_id" required className="rounded-lg border border-line bg-paper px-3 py-1.5">{options.map((option) => <option key={option.id} value={option.id}>{option.title.en}</option>)}</select></label>}
    </>
  );
}
